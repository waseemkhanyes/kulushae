...
extension GQLK {
  class Update_motorsMutation: GraphQLMutation {
    static let operationName: String = "Update_motors"
    static let operationDocument: ApolloAPI.OperationDocument = .init(
      definition: .init(
        #"mutation Update_motors($itemId: Int, $args: JSON) { update_motors(item_id: $itemId, args: $args) { __typename status message } }"#
      ))
    ...
    struct Data: GQLK.SelectionSet {
      ...
      static var __selections: [ApolloAPI.Selection] { [
        .field("update_motors", Update_motors?.self, arguments: [
          "item_id": .variable("itemId"),
          "args": .variable("args")
        ]),
      ] }

      var update_motors: Update_motors? { __data["update_motors"] }

      /// Update_motors
      ///
      /// Parent Type: `UpdateResponse`
      struct Update_motors: GQLK.SelectionSet {
        let __data: DataDict
        init(_dataDict: DataDict) { __data = _dataDict }
