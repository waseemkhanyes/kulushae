...
extension GQLK {
  class PropertiesQuery: GraphQLQuery {
    static let operationName: String = "Properties"
    static let operationDocument: ApolloAPI.OperationDocument = .init(
      definition: .init(
        #"query Properties($page: Int, $userId: Int, $categoryId: Int, $filters: JSON) { properties( page: $page user_id: $userId category_id: $categoryId filters: $filters ) { __typename current_page per_page total data { __typename id country emirates title contact_number price description socialmedia { __typename id type value } size total_closing_fee bedrooms bathrooms developer ready_by annual_community_fee furnished reference_number buyer_transfer_fee seller_transfer_fee maintenance_fee occupancy_status amenities { __typename id title } posted_by user_id { __typename id image first_name last_name email phone member_since total_listings } neighbourhood location category_id images { __typename id image } is_featured is_favorite type deposit } } }"#
      ))
    ...
    struct Data: GQLK.SelectionSet {
      ...
      static var __selections: [ApolloAPI.Selection] { [
        .field("properties", Properties?.self, arguments: [
          "page": .variable("page"),
      ...
      var properties: Properties? { __data["properties"] }

      /// Properties
      ///
      /// Parent Type: `PropertiesResponse`
      struct Properties: GQLK.SelectionSet {
        let __data: DataDict
        init(_dataDict: DataDict) { __data = _dataDict }

        static var __parentType: any ApolloAPI.ParentType { GQLK.Objects.PropertiesResponse }
        static var __selections: [ApolloAPI.Selection] { [
