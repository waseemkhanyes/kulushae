//
//  MotorChatViewDetails.swift
//  Kulushae
//
//  Created by ios on 06/04/2024.
//
...
struct MotorChatViewDetails: View {
    @StateObject var productDataHandler = CarDetailsViewModel.ViewModel()
    ...
    var body: some View {
        ZStack(alignment: .bottom) {
            ...
            NavigationLink("", destination: CarDetailsView(motorId: productId, fromChat: true), isActive: $isDetailViewActive)
            ...
            VStack(spacing: 0) {
                ...
                HStack() {
                    AsyncImage(url: URL(string: (Config.imageBaseUrl) + (productDataHandler.motorObject.images?.first?.image ?? "") )) { image in
                        image
                            .resizable()
                    ...
                    VStack(alignment: .leading) {
                        Spacer()
                        Text(LocalizedStringKey(productDataHandler.motorObject.title ?? ""))
                            .font(.roboto_20())
                            .font(.headline.weight(.semibold))
                            .environment(\.locale, .init(identifier: languageManager.currentLanguage))
                            .multilineTextAlignment(.leading)
                            .foregroundColor(Color.black)
                        Text(String(format: "%.2f", productDataHandler.motorObject.price ?? 0.0))
                            .font(.roboto_20())
                            .font(.headline.weight(.semibold))
                            .multilineTextAlignment(.leading)
                            .foregroundColor(Color.black)
                            .padding(.bottom, 15)
                        Text(LocalizedStringKey(productDataHandler.motorObject.emirates ?? ""))
                            .font(.roboto_14())
                            .environment(\.locale, .init(identifier: languageManager.currentLanguage))
                            .multilineTextAlignment(.leading)
                ...
                .onChange(of: uploadedImages) { isUploading in
                    ...
                    dataHandler.sentMotorMessage(serviceType: trim(productDataHandler.motorObject.type),
                                                 type: selectedImagePickerType,
                                                 message: uploadedImages.map { $0.fileName }.joined(separator: ","),
                                                 itemData: productDataHandler.motorObject,
                                                 receiverId: isFromNotification ? receiverId : productDataHandler.motorObject.userID?.id ?? "",
                                                 chatId:  dataHandler.chatIdVal != "" ?  dataHandler.chatIdVal : chatId
                                                 
                    )
                    DispatchQueue.main.asyncAfter(deadline: .now() + 0.2) {
                        let messageData = ChatMessage(
                            receiverID: Int(isFromNotification ? receiverId : productDataHandler.motorObject.userID?.id ?? "") ?? 0,
        ...
        .onAppear() {
            productDataHandler.getMotorDetails(request: CarDetailsViewModel.GetCarDetailsRequest.Request(carId: self.productId))
        ...
        .onChange(of: productDataHandler.motorObject) { motorObject in
            if(!isFromNotification) {
                dataHandler.setupPusher(senderId: PersistenceManager.shared.loggedUser?.id  ?? "",
                                        catId: "\(productDataHandler.motorObject.categoryID ?? 0)",
                                        itemId: "\(self.productId)",
                                        receiverId: productDataHandler.motorObject.userID?.id ?? "0")
                dataHandler.getOldChatMessages(senderId: PersistenceManager.shared.loggedUser?.id  ?? "",
                                               catId: "\(productDataHandler.motorObject.categoryID ?? 0)",
                                               itemId: "\(self.productId)",
                                               receiverId: productDataHandler.motorObject.userID?.id ?? "0",
