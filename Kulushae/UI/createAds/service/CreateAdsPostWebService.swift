...
class CreateAdsPostWebService {
    ...
    func submitAdvertisement(request: [String: Any?],
                             completion: @escaping (CreateAdsFormViewModel.MakeAdSubmit.Response) -> Void = { _ in })  {
                ...
                Network.shared.apollo.perform(mutation: GQLK.Store_propertyMutation(args: jsonString.graphQLNullable)) { result in
                    ...
                    case .success(let graphQLResult):
                        ...
                        if let response = graphQLResult.data?.store_property {
                            ...
                            CreateAdsFormViewModel.MakeAdSubmit.Response(
                            ...
                            for graphQLError in errors {
                                ...
                                let response = CreateAdsFormViewModel.MakeAdSubmit.Response(response: SubmitResponseModel(data: nil, errors: [submitError]))
                                completion(response)
                            }
                        }
                    ...
                    case .failure(let error):
                        ...
                        let response = CreateAdsFormViewModel.MakeAdSubmit.Response()
                        completion(response)
                    }
                }
            }
    ...
    func updatePropertyAdvApi(formId: Int, request: [String: Any?],
                             completion: @escaping (CreateAdsFormViewModel.UpdatePropertySubmit.Response) -> Void = { _ in })  {
        if let jsonData = try? JSONSerialization.data(withJSONObject: request.removeNullValues, options: [.sortedKeys]) {
            // Step 1: Convert the JSON object to a string
            ...
            if let jsonString = String(data: jsonData, encoding: .utf8) {
                ...
                if let itemJsonData = try? JSONSerialization.data(withJSONObject: ["itemId": formId], options: [.sortedKeys]) {
                    if let itemJsonString1 = String(data: itemJsonData, encoding: .utf8) {
                        print("add submit params", Config.emptyData + itemJsonString1)
                        (Config.emptyData + itemJsonString1).createSignature()
                    }
                }
                
                Network.shared.apollo.perform(mutation: GQLK.Update_propertyMutation(itemId: formId.graphQLNullable, args: jsonString.graphQLNullable)) { result in
                    
                    switch result {
                    case .success(let graphQLResult):
                        
                        if let response = graphQLResult.data?.update_property {
                            
                            completion(CreateAdsFormViewModel.UpdatePropertySubmit.Response(response: UpdatePropertyFormResponseModel(message: response.message, status: response.status)))
                            
                            
                        } else if let errors = graphQLResult.errors {
//                            for graphQLError in errors {
//                                let submitError = SubmitErrorModel(graphQLError: graphQLError)
//                                let response = CreateAdsFormViewModel.UpdatePropertySubmit.Response(response: UpdatePropertyFormResponseModel(data: nil, errors: [submitError]))
//                                completion(response)
//                            }
                        }
                    ...
                    case .failure(let error):
                        // Handle the network error
                        print("failure errr", error)
                        let response = CreateAdsFormViewModel.UpdatePropertySubmit.Response()
                        completion(response)
                    }
                }
            }
    ...
    func updateMotorCars(formId: Int, request: [String: Any?],
                         completion: @escaping (CreateAdsFormViewModel.UpdateMotorSubmit.Response) -> Void = { _ in })  {
        if let jsonData = try? JSONSerialization.data(withJSONObject: request.removeNullValues, options: [.sortedKeys]) {
            if let jsonString = String(data: jsonData, encoding: .utf8) {
...
//                print("add submit params", Config.emptyData + jsonString)
//                (Config.emptyData + "{\"itemId:\(formId)}}").createSignature()
                
                
                Network.shared.apollo.perform(mutation: GQLK.Update_motorsMutation(itemId: formId.graphQLNullable, args: jsonString.graphQLNullable)) { result in
                    
                    switch result {
                    case .success(let graphQLResult):
                        
                        if let response = graphQLResult.data?.update_motors {
                            
                            completion(CreateAdsFormViewModel.UpdateMotorSubmit.Response(response: UpdateMotorFormResponseModel(message: response.message, status: response.status)))
                            
                        } else if let errors = graphQLResult.errors {
//                            for graphQLError in errors {
//                                let submitError = SubmitErrorModel(graphQLError: graphQLError)
//                                let response = CreateAdsFormViewModel.UpdateMotorSubmit.Response()
//                                completion(response)
//                            }
                        }
    ...
    func deleteFormImageApi(serviceType: String, imageId: String, completion: @escaping (JSON, Error?) -> Void) {
        let params = [
        ...
        RestAPINetworkManager.shared.callEndPoint(
            url: Config.baseURL + Config.removeItemImage,
            parameters: params,
            headers: [
                "X-App-Language": UserDefaults.standard.string(forKey: "AppLanguage") ?? "en",
                "Authorization": "Bearer \(UserDefaults.standard.string(forKey: Keys.Persistance.authKey.rawValue) ?? "")"
            ]
